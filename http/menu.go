...
func GetMenuDetail(c *gin.Context) {
	...
	resp.Data, err = ucmodel.GetMenuByID(idStr)
	if err != nil {
		resp.Code = model.InternalServerError
		resp.Message = err.Error()
	}
...
func GetMenuTree(c *gin.Context) {
	resp := &ucmodel.QueryMenuResponse{
		CommonResponse: model.CommonResponse{
			Code: model.Success,
		},
	}
	var err error
	resp.Data, resp.Records, err = ucmodel.GetInfoList()
	if err != nil {
		resp.Code = model.InternalServerError
		resp.Message = err.Error()
	}
